...
class Omni3DNode():
    ...
    def __init__(self, args):
        ...
        cam0_sub = Subscriber(cameraTopics[0], Image)
        cam1_sub = Subscriber(cameraTopics[1], Image)
        cam2_sub = Subscriber(cameraTopics[2], Image)
        cam3_sub = Subscriber(cameraTopics[3], Image)
        self.camNum = 4
        ...
        omni3dTopic = rospy.get_param('omni3dTopic')
        self.pred_pub = rospy.Publisher(omni3dTopic, Float32MultiArray, queue_size=1)
        self.pred_pub_debug = rospy.Publisher('omni3d_debug', Image, queue_size=1)
        self.marker_pub = rospy.Publisher("omni3dMarkerTopic", MarkerArray)
...
def main():
    ...
    rospy.init_node('Omni3DNode', anonymous=True)
    #rate = rospy.Rate(10)
    args = parse()
    omn = Omni3DNode(args)
    rospy.spin()
